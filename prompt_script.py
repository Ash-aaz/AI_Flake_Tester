...
class Flake_Tester:
    def __init__(self, agent, total_count, prompt_difficulty):
        self.agent = agent
    ...
    def validate_json(self, json_outputs: list):
        flake_counter = 0
        if (self.prompt_difficulty == 'easy'):
            user_list_adapter = TypeAdapter(list[User])
    
        elif (self.prompt_difficulty == 'hard'):
            user_list_adapter = TypeAdapter(Logs)
    
        else:
            user_list_adapter = TypeAdapter(list[Employee])
        ...
        for output in json_outputs:
            if (output is None):
             flake_counter += 1
                ...
                try:
                    user_list_adapter.validate_json(output, strict= True)
                except ValidationError:
                    flake_counter += 1
    
        print("Flake Counter = ", flake_counter)
    ...
    def model_efficiency(self, duration_values: list, count_values: list):
        ...
        while single_tps < self.total_count:
            if (duration_values[single_tps] <= 0):
                single_tps += 1
                current_count -= 1
                continue
            tps = (count_values[single_tps]) / (duration_values[single_tps] * (10**-9))
            total_tps += tps
            single_tps += 1
    
        if (current_count == 0):
            print("All prompts failed to generate a valid output.")
        else:
            avg_tps = total_tps / current_count
            print("Model's average token generation speed is: ", avg_tps)
            return avg_tps

    async def generate(self):
        if (self.prompt_difficulty == 'easy'):
            message =  """You are a strict data extraction API. 
        ...
        response = await AsyncClient().generate(model=self.agent, prompt=message)

        return (response['response'], response['eval_duration'], response['eval_count'])

    async def main(self):
        task = [asyncio.create_task(self.generate()) for _ in range(self.total_count)]
        ...
        for unpacker in results:
            json_object, duration_object, count_object = unpacker
            json_outputs.append(json_object)
            duration_outputs.append(duration_object)
            count_outputs.append(count_object)

        print("Data Generated:", len(results))
...
if __name__ == "__main__":
    ...
    tester = Flake_Tester(args.model, args.run, args.difficulty)
