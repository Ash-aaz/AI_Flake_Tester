...
# Test if validate JSON flags incorrect outputs and lets ideal outputs pass through at all difficulty tiers
def test_validate_json_easy_ideal():
    ...
    output = validate_json(ideal_output, TypeAdapter(Logs))
    assert output.flake_counter == 0
    assert output.error_distribution == {}
...
def test_validate_json_easy_false(incorrect_output):
    output = validate_json(incorrect_output, TypeAdapter(Logs))
    assert output.flake_counter == 1
    assert output.error_distribution != {}
    assert output.failed_outputs != []
...
def test_validate_json_med_ideal():
    ...
    output = validate_json(ideal_output, TypeAdapter(Memo))
    assert output.flake_counter == 0
    assert output.error_distribution == {}
    assert output.failed_outputs == []
...
def test_validate_json_med_false(incorrect_output):
    output = validate_json(incorrect_output, TypeAdapter(Memo))
    assert output.flake_counter == 1
    assert output.error_distribution != {}
    assert output.failed_outputs != []
...
def test_validate_json_hard_ideal():
    ...
    output = validate_json(ideal_output, TypeAdapter(Breach))
    assert output.flake_counter == 0
    assert output.error_distribution == {}
    assert output.failed_outputs == []
...
def test_validate_json_hard_false(incorrect_output):
    output = validate_json(incorrect_output, TypeAdapter(Breach))
    assert output.flake_counter == 1
    assert output.error_distribution != {}
    assert output.failed_outputs != []
...
def test_validate_json_ultra_ideal():
    ...
    output = validate_json(ideal_output, TypeAdapter(Report))
    assert output.flake_counter == 0
    assert output.error_distribution == {}
    assert output.failed_outputs == []
...
def test_validate_json_ultra_false(incorrect_output):
    output = validate_json(incorrect_output, TypeAdapter(Report))
    assert output.flake_counter == 1
    assert output.error_distribution != {}
    assert output.failed_outputs != []

# Use dummy values to verify avg. tps is calculated correctly
...
# For less than 100 verify P95 and P99 aren't calculated
...
# Verify P95 and P99 are calculated correctly when sufficient data is present
